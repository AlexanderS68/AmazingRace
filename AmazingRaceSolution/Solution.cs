namespace AmazingRaceSolution{
    public class Solution
    {
        static void Main() {
            ...
            var start = new Point(0, Array.IndexOf(maze[0], 1));
            var end = new Point(maze.Length - 1, Array.LastIndexOf(maze[^1], 1));

            // Find a path from the start to the end points
            var path = FindPath(maze, start, end, new List<Point> { start });
            if (path != null) {
        ...
        // Recursive function to find a path from the start point to end point in the maze
